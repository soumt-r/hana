...
`run`は、標準ではより速いバイトコードコンパイラとVMで実行し、バイトコードが対応していない機能を使ったプログラムならツリーウォーク方式のインタープリタで実行します。`--tree`はツリーウォーク方式のインタープリタで、`--bc`は切り替えずにバイトコードだけで実行します。`-t`はどのエンジンで実行したかと、パースと実行にかかった時間を表示します。`--allow-file=false`と`--allow-net=false`で、`【ファイル】`、`【ソケット】`、`【HTTP】`モジュールを止められます。
