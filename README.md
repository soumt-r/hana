...
`run`은 기본으로 더 빠른 바이트코드 컴파일러와 VM으로 실행하고, 바이트코드가 지원하지 않는 기능을 쓴 프로그램이면 트리워킹 인터프리터로 실행해요. `--tree`는 트리워킹 인터프리터로, `--bc`는 바꾸지 않고 바이트코드로만 실행해요. `-t`는 어느 엔진으로 실행했는지와 파싱과 실행에 걸린 시간을 보여줘요. `--allow-file=false`와 `--allow-net=false`는 `[파일]`, `[소켓]`, `[HTTP]` 모듈을 막아요.
