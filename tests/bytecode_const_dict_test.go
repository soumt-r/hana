...
func runAllFour(t *testing.T, c dualRun) (outs [4][]string, errs [4]error) {
	t.Helper()
	i0, e0 := runHaja(t, c.haja)
	b0, e1 := runBytecode(t, c.haja)
...
func TestBytecodeConstantsCannotBeReassigned(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			for i, name := range []string{"haja tree", "haja bytecode", "kanade tree", "kanade bytecode"} {
...
`,
	...
	wantHaja := []string{"ConstantAssignmentError: 상수 '원주율'의 값은 변경할 수 없어요.", "원주율은 3.14"}
	wantKanade := []string{"ConstantAssignmentError: 定数 '円周率' の値は変更できません。", "円周率は3.14"}
	for i, want := range [][]string{wantHaja, wantHaja, wantKanade, wantKanade} {
...
func TestBytecodeDictionaries(t *testing.T) {
	c := dualRun{
		haja: `'점수'를 [(문자열, 숫자)사전]인 {"국어": 90, "수학": 80}으로 정하자
...
`,
	...
	wantHaja := []string{"국어 점수: 90", "오른 수학 점수: 90", "영어: 95", "KeyError: 사전에서 '과학' 이름을 찾을 수 없어요."}
...
func TestBytecodeDictionaryIsSharedByReference(t *testing.T) {
	c := dualRun{
		haja: `<사전바꾸기>를 만들자 ([(문자열, 아무거나)사전]인 '사전'):
    '사전'의 "상태"를 "바뀜"으로 정하자
