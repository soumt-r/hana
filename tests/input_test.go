...
// runAllFourWithInput runs code on tree-walker/bytecode x haja/kanade, feeding each the same stdin.
func runAllFourWithInput(t *testing.T, haja, kanade, stdin string) (outs [4][]string, errs [4]error) {
	t.Helper()
	{
		p := parser.New(lexer.New(haja))
		prog := p.ParseProgram()
...
func TestInputReadsAndConvertsOnEveryEngine(t *testing.T) {
	...
	kanade := "『名前』を入力してもらおう\n『年齢』を【数字】で入力してもらおう\n『はい』を【論理】で入力してもらおう\n枠「{『名前』}/{『年齢』 + 1}/{『はい』}」を出力しよう\n"
	outs, errs := runAllFourWithInput(t, haja, kanade, "홍길동\n 20 \n참\n")
	// Kanade's boolean word is 真, so 참 is not a boolean there.
	if errs[0] != nil || errs[1] != nil {
		t.Fatalf("haja: tree %v, bytecode %v", errs[0], errs[1])
	}
	for i := 0; i < 2; i++ {
		if strings.Join(outs[i], "|") != "홍길동/21/참" {
			t.Errorf("haja engine %d output = %v", i, outs[i])
		}
	}
	...
	outs, errs = runAllFourWithInput(t, haja, kanade, "田中\n 20 \n真\n")
	for i := 2; i < 4; i++ {
...
func TestInputConversionFailureIsACatchableError(t *testing.T) {
	haja := "일단 해보자:\n    '나이'를 [숫자]로 입력받자\n오류가 발생했다면 ('에러'):\n    '에러'의 '메시지'를 출력하자\n"
	kanade := "とりあえずやってみよう:\n    『年齢』を【数字】で入力してもらおう\n発生したら(『エラー』):\n    『エラー』の『メッセージ』を出力しよう\n"
	outs, errs := runAllFourWithInput(t, haja, kanade, "스물\n")
	for i := range outs {
		if errs[i] != nil {
	...
	if len(outs[0]) != 1 || !strings.Contains(outs[0][0], "숫자") || strings.Join(outs[0], "") != strings.Join(outs[1], "") {
		t.Errorf("haja messages: tree %v, bytecode %v", outs[0], outs[1])
	}
}

func TestInputWithoutASourceReadsAnEmptyLine(t *testing.T) {
	i, err := runHaja(t, "'이름'을 입력받자\n틀\"[{'이름'}]\"를 출력하자\n")
