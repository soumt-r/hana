...
// interfaces' types. Every case runs on tree-walker/bytecode x haja/kanade.
...
func runTypeCases(t *testing.T, cases []typeCase) {
	t.Helper()
	labels := []string{"haja tree", "haja bytecode", "kanade tree", "kanade bytecode"}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			outs, errs := runAllFour(t, dualRun{name: c.name, haja: c.haja, kanade: c.kanade})
...
func TestGenericsAreCheckedWhenElementsAreAdded(t *testing.T) {
	runTypeCases(t, []typeCase{
		...
		},
		{
			...
			haja:    "'점수'를 [(문자열, 숫자)사전]인 {\"국어\": \"구십\"}으로 정하자\n",
			kanade:  "『点数』を【(文字列,数字)辞書】の{「国語」:「九十」}にしよう\n",
			wantErr: "can only hold",
		},
		{
			name:   "a well-typed dictionary",
			haja:   "'점수'를 [(문자열, 숫자)사전]인 {\"국어\": 90}으로 정하자\n'점수'의 \"국어\"를 출력하자\n",
	...
	runTypeCases(t, []typeCase{
		{
			...
			haja:       hajaAnimals + "'친구'를 [동물]인 새로운 [강아지]()로 정하자\n'친구'의 '재주'를 출력하자\n",
