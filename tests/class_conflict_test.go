...
func TestTwoModulesWithTheSameClassNameConflict(t *testing.T) {
	inTempDir(t, map[string]string{"a.hj": boxA, "b.hj": boxB})
	code := "\"a.hj\"에서 '상자'를 가져오자\n\"b.hj\"에서 '상자'를 가져오자\n"
...
func TestAModuleClassConflictsWithAClassOfTheProgram(t *testing.T) {
	inTempDir(t, map[string]string{"a.hj": boxA})
	code := "\"a.hj\"에서 '상자'를 가져오자\n" + boxB
	_, err := runHaja(t, code)
...
func TestTheSameModuleImportedTwiceIsNoConflict(t *testing.T) {
	inTempDir(t, map[string]string{"a.hj": boxA + "<이름>을 만들자 ():\n    \"a\"를 돌려주자\n"})
	code := "\"a.hj\"에서 '상자'를 가져오자\n\"a.hj\"에서 <이름>을 가져오자\n<이름>()을 출력하자\n"
