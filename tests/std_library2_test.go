...
// (and random's UUID), on tree-walker/bytecode x hari/kanade. The docs sites'
...
func hariImports(module string, names ...string) string {
	out := ""
	for _, n := range names {
...
func TestStatsModule(t *testing.T) {
	...
	runTypeCases(t, []typeCase{
		...
		},
		{
			...
			hari:    h + "<표준편차>([5])를 출력하자\n",
			kanade:  k + "〈標準偏差〉(【5】)を出力しよう\n",
			wantErr: "not enough values",
		},
		{
			name:    "only numbers",
			hari:    h + "<최댓값>([1, \"둘\"])을 출력하자\n",
