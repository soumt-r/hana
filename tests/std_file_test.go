...
func TestFileAccessPolicy(t *testing.T) {
	...
	for _, call := range []struct{ h, k string }{
		...
		cases = append(cases, typeCase{name: call.h, hari: h + call.h, kanade: k + call.k, wantErr: "File access is turned off"})
	}
