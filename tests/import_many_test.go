...
const libHaja = `[숫자]를 돌려주는 <두배>를 만들자 ([숫자]인 '값'):
    ('값' * 2)를 돌려주자
...
func TestMissingItemInAFileListIsAnError(t *testing.T) {
	inTempDir(t, map[string]string{"lib.hj": libHaja})
	code := "\"lib.hj\"에서 <두배>와 <없는것>을 가져오자\n"
