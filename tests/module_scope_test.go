...
const scopedMain = `
"lib.hj"에서 <더하기>를 가져오자
"lib.hj"에서 <설명하기>를 가져오자
"lib.hj"에서 '상자'를 가져오자
...
func TestImportedCodeUsesItsOwnModulesFunctions(t *testing.T) {
	inTempDir(t, map[string]string{"lib.hj": scopedLib})
	want := "15|값의 두배는 8|18|8|-1"

	tree, err := runHaja(t, scopedMain)
	if err != nil {
...
func TestAModuleUsesWhatItImportedItself(t *testing.T) {
	inTempDir(t, map[string]string{
		"low.hj": "<낮은>을 만들자 ('값'):\n    ('값' + 100)를 돌려주자\n",
		"mid.hj": "\"low.hj\"에서 <낮은>을 가져오자\n<중간>을 만들자 ('값'):\n    '결과'를 <낮은>('값')로 정하자\n    '결과'를 돌려주자\n",
	})
	main := "\"mid.hj\"에서 <중간>을 가져오자\n<중간>(1)을 출력하자\n"
	tree, err := runHaja(t, main)
	if err != nil {
		t.Fatalf("tree-walker: %v", err)
	}
...
func TestAPackageUsesItsOwnHelpersAndItsDependencies(t *testing.T) {
	...
	installed(t, map[string]string{
		title + "@2.0.0/haja/index.hj": "<제목붙이기>를 만들자 ('글'):\n    '결과'를 틀\"== {'글'} ==\"으로 정하자\n    '결과'를 돌려주자\n",
		greetPath + "@1.0.0/haja/index.hj": "[" + title + "]에서 <제목붙이기>를 가져오자\n" +
			"<큰소리>를 만들자 ('글'):\n    (('글' + \"!\"))를 돌려주자\n" +
