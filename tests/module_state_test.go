...
func TestAModuleIsSetUpOnceAndItsStateIsShared(t *testing.T) {
	inTempDir(t, map[string]string{
		"counter.hr": "\"카운터를 준비해요\"를 출력하자\n'횟수'를 [숫자]인 0으로 정하자\n<세기>를 만들자 ():\n    '횟수'에 1을 더하자\n    '횟수'를 돌려주자\n",
		"mid.hr":     "\"counter.hr\"에서 <세기>를 가져오자\n<중간세기>를 만들자 ():\n    <세기>()를 돌려주자\n",
	})
	code := "\"counter.hr\"에서 <세기>를 가져오자\n\"mid.hr\"에서 <중간세기>를 가져오자\n<세기>()를 출력하자\n<중간세기>()를 출력하자\n<세기>()를 출력하자\n"
	tree, err := runHari(t, code)
	if err != nil {
		t.Fatalf("tree-walker: %v", err)
	}
...
func TestAPackageKeepsItsStateToo(t *testing.T) {
	installed(t, map[string]string{
		greetPath + "@1.0.0/hari/index.hr": "'인사말'을 [문자열]인 \"안녕\"으로 정하자\n<말하기>를 만들자 ('이름'):\n    틀\"{'인사말'}, {'이름'}!\"을 돌려주자\n<바꾸기>를 만들자 ('새말'):\n    '인사말'을 '새말'로 정하자\n",
	}, map[string]string{
