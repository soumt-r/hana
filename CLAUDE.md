...
- **`hana run`의 기본 엔진은 바이트코드**입니다(`cmd/run.go`): 컴파일에 실패하는 프로그램(블록 안의 함수 선언처럼 바이트코드가 지원하지 않는 것)만 트리워커로 돌고, `--bc`는 폴백 없이 바이트코드만, `--tree`는 트리워커를 씁니다. `-t`가 쓴 엔진을 보여 줍니다(`tests/run_engine_test.go`). 두 docs의 `compare_tests.ts`가 `hana run`과 비교하므로 docs 예제는 바이트코드로 대조됩니다(바꾼 시점에 docs 예제 276개 모두 바이트코드로 컴파일됨). `doctest`는 트리워커 API를 직접 씁니다.
- **함수 호출 비용**: fib 한 번 호출이 약 100ns이고 실행하는 명령은 3~6개뿐이라, 대부분은 호출의 준비·정리입니다(인자 묶기, 반환 타입 검사, 프레임 반납이 각각 수 %). 그중 `checkedReturn`의 `QuickAccepts` 빠른 경로, `CALL` 뒤 인자 자리 `clear`를 하지 않기(다음 push가 덮어씀), 인자 수가 맞을 때의 `bindArgs` 빠른 경로, 새 프레임에 매개변수를 찾지 않고 붙이기(`frame.bindParam`), `varSlot` 필드 재배치(56→48바이트)를 합쳐 fib가 약 8% 빨라졌습니다(하나씩은 잡음에 묻혀 안 보임 — 함수 호출은 `-bench Programs/fib/bytecode -benchtime 20x -count 10 -cpu 1`로 benchstat 하세요). `Symbols()` 인라인과 피연산자 스택 초기 크기 8은 차이가 없었습니다. `run`의 스택 프레임(약 1.8KB)은 입구에서 0으로 채우는 것이 피연산자 스택 256바이트뿐이라 호출 비용의 원인이 아닙니다.
- **해 보았지만 이득이 없었던 것**(다시 하지 마세요): ① `run`의 `push`/`pop` 클로저를 없애기 — 프로파일에서 `pop`이 약 10%로 보였지만 인라인해도 그대로였습니다(pprof는 인라인된 함수도 따로 보여 주므로 그건 호출 비용이 아니라 값 꺼내기 자체). 또 포인터 메서드로 `*s = append(*s, v)`를 하면 탈출 분석이 피연산자 스택 배열을 힙으로 보내 fib가 2.6배 느려집니다. ② 명령마다의 `instr.Operand.(int)`를 청크별 정수 표로 바꾸기 — classes −6%, fib +6%(호출마다 표를 찾는 비용)로 합계 0. 명령 하나의 고정 비용은 병목이 아니고, 남은 큰 비용은 `num.Box`(숫자를 `interface{}`에 담기)와 이름으로 변수 찾기(`probe`/`frame.put`), 객체 필드 맵(`newObject`)입니다.
...
반복 변수(범위의 카운터·반복 변수, `마다`의 항목)와 처리기의 오류 변수는 **자기 범위에 새로 선언**되어 같은 이름의 바깥 변수를 가리고, 범위가 끝나면 바깥 것이 그대로 돌아옵니다(`'가'`가 100일 때 `1부터 2까지 반복하자 ('가')` 뒤의 `'가'`는 100). 바이트코드는 `DECLARE_VAR`(이미 있어도 새 칸, `frame.declare`)로 선언하고, 가려진 칸은 `hidden`(힌트 캐시 `probe`도 건너뜀), 새 칸은 `shadows`에 가린 칸을 적어 `POP_SCOPE`(`dropFrom`)가 되살립니다. 이름 찾기(`frame.find`)는 뒤에서부터라 가장 최근 선언이 이깁니다. 범위의 양 끝은 반복 변수를 선언하기 전에 계산합니다(`1부터 '가'까지 반복하자 ('가')`의 끝은 바깥 `'가'`). 오류가 반복·처리기를 뚫고 나가 `일단 해보자`에 잡히면 `POP_SCOPE`를 건너뛰므로, `TRY_PUSH` 때의 프레임 크기(`tryHandler.frameDepth`) 위에 남은 첫 범위 표식(`varSlot.marker`)부터 버립니다(`unwindScopes`, `일단 해보자` 본문이 직접 선언한 변수는 남음). `반복을 끝내자`는 **그 반복 안에서 들어간** `일단 해보자`만 빠져나옵니다(컴파일러의 `loopTries`와 `emitTryExitsTo`; 예전엔 반복을 둘러싼 `일단 해보자`까지 `TRY_POP`해서 VM이 빈 try 스택에서 패닉했음, `tests/break_try_test.go`). 반복 밖에서 `반복을 끝내자`가 **실행되면** 그 자리에서 `IllegalBreakError`입니다(스펙 4.4, `errs.IllegalBreak`, `일단 해보자`로 잡을 수 있고 실행되지 않으면 아무 일도 없음): 트리워커는 반복 범위(`newLoopScope`, `Environment.loop`)를 범위 사슬에서 찾아(`inLoop`, 사슬이 어휘적이라 함수를 부른 쪽의 반복은 세지 않음 — 예전엔 함수 안의 `반복을 끝내자`가 부른 쪽의 반복을 끊었음), 바이트코드는 `ILLEGAL_BREAK`로 컴파일합니다(예전엔 컴파일 오류로 보고하려다 최적화 단계가 목표 없는 `JUMP`에서 패닉). 처리기 안에서 오류가 나도 그 `일단 해보자`의 `마무리는 항상`이 돕니다: `dispatchError`가 마무리가 있으면 처리기로 들어갈 때 잡지는 않고 마무리만 하는 층을 다시 쌓고, 처리기 끝(과 `돌려주자`/`반복을 끝내자`, `inProtectedBlock`)이 그 층을 뺍니다. 범위의 양 끝이 숫자가 아니면 두 엔진 모두 `RangeMustBeNumbers`입니다(바이트코드는 리터럴이 아닌 범위에 `CHECK_RANGE`). 상수 여부는 그 이름을 가진 **가장 가까운 범위**가 정합니다(트리워커 `Environment.isConst` — 예전엔 부모까지 거슬러 올라가 가려진 바깥 상수 때문에 반복 변수에 `더하자`가 `ConstantAssignmentError`였음).
