...
const hariFunc = "[숫자]를 돌려주는 <더하기>를 만들자 ([숫자]인 '가', [숫자]인 '나'):\n    '결과값'을 [숫자]인 '가' + '나' 로 정하자\n    '결과값'을 돌려주자\n"
...
func TestHoverParameterShowsDeclaredType(t *testing.T) {
	got := hoverText(hariLang, hariFunc, 0, 30) // on the '가' parameter
...
func TestHoverLocalVariableShowsItsAnnotatedType(t *testing.T) {
	got := hoverText(hariLang, hariFunc, 1, 6)
...
func TestUnannotatedVariableHasNoTypeLine(t *testing.T) {
	got := hoverText(hariLang, "'나이'를 3으로 정하자\n", 0, 1)
...
func TestHoverFunctionShowsReturnType(t *testing.T) {
	got := hoverText(hariLang, hariFunc, 0, 12) // on <더하기>
...
func TestHoverNothingOnPlainText(t *testing.T) {
	if got := hoverText(hariLang, "\"안녕\"을 출력하자\n", 0, 2); got != "" {
...
func TestHoverRangeCoversTheTokenInUTF16Units(t *testing.T) {
	...
	if hoverAt(hariLang, src, 0, 2) != nil {
		t.Errorf("no keyword sits on the emoji")
	}
}

func TestHoverOverTheWire(t *testing.T) {
	open := `{"jsonrpc":"2.0","method":"textDocument/didOpen","params":{"textDocument":{"uri":"file:///a.hr","text":"\"a\"를 출력하자\n"}}}`
	req := `{"jsonrpc":"2.0","id":5,"method":"textDocument/hover","params":{"textDocument":{"uri":"file:///a.hr"},"position":{"line":0,"character":5}}}`
	miss := `{"jsonrpc":"2.0","id":6,"method":"textDocument/hover","params":{"textDocument":{"uri":"file:///a.hr"},"position":{"line":0,"character":1}}}`
